...
public class ParseStringPre2 {
	...
	public static void main(String[] args) {
		...
		boolean is주민 = (주민번호1.length() == 14) && // 1. 주민등록번호가 - 를 포함하여 14자리가 맞는지 확인하는 코드
				       (주민번호1.charAt(6) == '-') && // 2. 주민등록번호 생년월일 작성 후, - 가 들어가 있는지
				       (주민번호1.substring(0, 6).matches("[0-9]+")) && // 3. 앞의 6자리가 숫자로만 이루어져 있는지
				       (주민번호1.substring(7).matches("[0-9]+")); // 4. 뒤에서 7번째 자리부터 끝까지 모두 숫자로만 이루어져 있는지
